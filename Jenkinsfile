pipeline {
    ...
    stages {
        stage('Docker') {
            ...
            when {
                anyOf {
                    branch pattern: 'kurry-.*', comparator: "REGEXP";
                    buildingTag();
                }
            }
            environment {
                DOCKER_REGISTRY = 'ghcr.io'
                GITHUB_ORG = 'kurry-social'
            ...
            stages {
                stage('Prepare') {
                    steps {
                        script {
                            if (env.BRANCH_NAME.startsWith('kurry-')) {
                            ...
                            env.SOURCE_BASE_URL = "https://github.com/kurry-social/mastodon" // I'm lazy. Will fix it later
